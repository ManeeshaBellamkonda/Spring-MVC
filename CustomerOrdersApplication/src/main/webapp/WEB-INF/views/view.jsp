													...
													.ajax({
														...
														success : function(
															...
															for (var i = 0; i < result.length; i++) {
																s += '<option value="'+result[i].orderId+'">'
																		+ result[i].orderName
																		+ '</option>';
															}
															console
...
<body>
	<form action="allData" method="get">
		<table id="tt" align="center">
			...
			<tr>
				<td>Orders:</td>
				<td><select name="orders" id="comboOrders" multiple="multiple">
		...
		<table class="table tale-stripped">
			...
			<c:forEach var="list" items="${orders}">
				<tr class="success">
					...
					<td><a href="/update/${list.orderId}">Update</a></td>
					<c:set var="sum" value="${sum+list.orderPrice}"></c:set>
				</tr>
			</c:forEach>
			<tr>
